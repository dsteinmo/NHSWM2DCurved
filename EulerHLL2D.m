function flux = EulerHLL2D(nx, ny, QM, QP, g)
...
[fxQM,fyQM,rhoM,uM,vM,pM] = EulerFluxes2D(QM, g);
[fxQP,fyQP,rhoP,uP,vP,pP] = EulerFluxes2D(QP, g);
EnerM = QM(:,:,4); EnerP = QP(:,:,4);

Nfields = 4;
HM   = (EnerM+pM)./rhoM; 
%cM = sqrt(gamma*pM./rhoM);
cM = sqrt(g*rhoM);
HP   = (EnerP+pP)./rhoP; 
%cP = sqrt(gamma*pP./rhoP);
cP = sqrt(g*rhoP);

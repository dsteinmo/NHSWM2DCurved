function flux = EulerLF2D(lnx, lny, QM, QP, g)
...
[fxM,fyM,rhoM,uM,vM,pM] = EulerFluxes2D(QM, g);
[fxP,fyP,rhoP,uP,vP,pP] = EulerFluxes2D(QP, g);
...
maxvel = max( sqrt(uM.^2+vM.^2) + sqrt(g*rhoM),  ...
	      sqrt(uP.^2+vP.^2) + sqrt(g*rhoP));

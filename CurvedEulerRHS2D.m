...
for n=1:3, cQ(:,:,n) = cub.V*Q(:,:,n); end;

% 1.2 Evaluate flux function at cubature nodes
%gamma = 1.4;
[F,G,rho,u,v,p] = EulerFluxes2D(cQ, g);
...
switch fluxtype
  case {'LF'}
    [flux] = EulerLF2D (nx, ny, gQM, gQP, g); 
  case {'Roe'}
    [flux] = EulerRoe2D(nx, ny, gQM, gQP, gamma);  %this one hasn't been modified for SW
  case {'HLL'}
    [flux] = EulerHLL2D(nx, ny, gQM, gQP, g); 
  case {'HLLC'}
    [flux] = EulerHLLC2D(nx, ny, gQM, gQP, gamma);  %this one is not implemented at all, anywhere (maybe my 1D SW code)
end
...
%Add Bathymetry source terms to rhs (Derek)
%make sure Hx & Hy computed at cubature nodes
rhsQ(:,:,2) = rhsQ(:,:,2) + g*Q(:,:,1).*Hx;
rhsQ(:,:,3) = rhsQ(:,:,3) + g*Q(:,:,1).*Hy;

%Add f-plane source terms to rhs
rhsQ(:,:,2) = rhsQ(:,:,2) + fRot*Q(:,:,3);
rhsQ(:,:,3) = rhsQ(:,:,3) - fRot*Q(:,:,2);
%disp('hi');
